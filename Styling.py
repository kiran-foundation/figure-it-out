...
def figure_it_out():
    ...
    if bust_floatval - hip_floatval <= 1 and hip_floatval - bust_floatval <= 3.6 and bust_floatval - waist_floatval >= 9 or hip_floatval - waist_floatval >= 10:
        print("Your body size match to: HOURGLASS category")
        print("These types of pattern suit on you: Narrow v neck, Dark top, Jackets, Tailor shirts. For more info, "
              "click below link: ")
        print("https://40plusstyle.com/how-to-dress-the-hourglass-shape/")
        repeat()

    # For Bottom hourglass
    if 3.6 <= hip_floatval - bust_floatval < 10 and hip_floatval - waist_floatval >= 9 and highhip_floatval / waist_floatval < 1.193:
        print("Your body size match to BOTTOM HOURGLASS category")
        print("These types of pattern suit on you: <fill dressing types here>. For more info, "
              "click below link: ")
        print("")
        repeat()

    # For Top hourglass
    if 1 < bust_floatval - hip_floatval < 10 and bust_floatval - waist_floatval >= 9:
        print("Your body size match to TOP HOURGLASS category")
        print("These types of pattern suit on you: Narrow V-necks. You should also try wearing dark tops. A couple of "
              "jackets and well-tailored shirts. For more info, click below link: ")
        print("")
        repeat()

    # For Spoon
    if hip_floatval - bust_floatval > 2 and hip_floatval - waist_floatval >= 7 and highhip_floatval / waist_floatval >= 1.193:
        print("Your body size match to SPOON category")
        print("These types of pattern suit on you: A-line skirts, boot cut jeans or trousers, mid-rise jeans, "
    ...
    # For Triangle
    if hip_floatval - bust_floatval >= 3.6 and hip_floatval - waist_floatval < 9:
        print("Your body size match to TRIANGLE category")
        print("SUITABLE STYLE : Wear bright scoop-neck and boat-neck tops. This will make your shoulders look wider. "
    ...
    if bust_floatval - hip_floatval >= 3.6 and bust_floatval - waist_floatval < 9:
        print("Your body size match to INVERTED TRIANGLE body shape")
        print("These types of pattern suit on you: Wear soft-textured fabrics, A-line dresses, shirts with a tie "
    ...
    if hip_floatval - bust_floatval < 3.6 and bust_floatval - hip_floatval < 3.6 and bust_floatval - waist_floatval < 9 and hip_floatval - waist_floatval < 10:
        ...
        print("Your body size match to Rectangle body shape, SUITABLE STYLE :Choose asymmetrical cuts to create "
        ...
        print("http://www.google.com")  # site URL
    ...
    if waist_floatval - bust_floatval >= 1 and hip_floatval - bust_floatval < 2:
        print("Your body size match to Diamond shape, This body shape has a thicker waist circumference than the bust "
              "round or hip round.The stomach area may appear protruding. Some may call it an Apple when the upper "
              "body is slightly bigger than the hip. SUITABLE STYLE : you should dress in such a way to hide your "
              "tummy by jackets, dupattas or draping style dress. For more info, Click below link")
        print("URL for Diamond")
        repeat()
    ...
    if bust_floatval - waist_floatval <= 0 and bust_floatval - hip_floatval > 1:
        print("Your body size match to OVAL body shape, Your waist is broader than your shoulders or hips. You’re "
              "generally well-proportioned. You have a fuller bust/upper body. Your waist is undefined. SUITABLE "
              "STYLE : Opt for silhouettes without structured waistlines to take focus away from the mid-section. "
